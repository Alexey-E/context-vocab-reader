...
public.sample_documents
  └─ curated, read-only demo content
```
...
Stores curated texts that demonstrate the reader before a visitor signs in. Samples are application-owned content and are intentionally separate from private user documents.
...
| ----------------- | ------------- | ---------------------------- |
| `id`              | `uuid`        | Primary key                  |
| `slug`            | `text`        | Stable unique URL identifier |
| `title`           | `text`        | Required                     |
| `content`         | `text`        | Curated source text          |
| `source_language` | `varchar(10)` | Language of the source text  |
| `target_language` | `varchar(10)` | Default translation language |
| `sort_order`      | `integer`     | Stable presentation order    |
| `created_at`      | `timestamptz` | Creation time                |

Initial content should cover three or four source languages and use stable slugs. It is versioned application data rather than user-generated content.

Reading position does not belong on a shared sample row. Guest progress can remain browser-local; persisted progress can be introduced later as a separate user-owned record if needed.
...
`sample_documents` is read-only through the Data API:

- `anon` and `authenticated` receive `select` only
- both roles can read curated rows
- neither role receives `insert`, `update`, or `delete`
- sample maintenance is performed through versioned migrations or another trusted administrative path
...
0003_public_sample_documents.sql
0004_seed_public_sample_documents.sql
```
...
Curated public sample content is versioned in a data migration so that local and remote databases receive the same rows. `seed.sql` remains local-only and contains development users and their private fixture data; it must not be applied to the remote project.
